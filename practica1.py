...
# Define la carpeta donde están todos los imágenes
ASSETS_DIR = "assets"
...
def game_over(screen, clock, score):
    ...
    text = font.render(f"¡Rumba destruido! Tuercas acumuladas: {score}", True, (255, 0, 0))
    screen.fill((0, 0, 0))
...
def load_and_scale_image(fname, size=None):
    """
    Carga una imagen usando pygame, la convierte a alpha y la escala
    si se indica un tamaño.
    """
...
def load_images_concurrently(fnames, size=None):
    """
    Carga una lista de imágenes de forma concurrente y las devuelve en el mismo orden.
    """
...
def main_menu(screen, clock, ancho, alto):
    """
    Muestra el menú principal del juego, con animación de sprites de fondo
    y opciones para seleccionar el modo de juego o salir.
    """
    # Rutas de los archivos de menú en la carpeta 'assets'
    ...
    option2 = option_font.render("2) Modo expectador (Auto)", True, (255, 255, 255))
...
def mover_rumba_simulation(
    ...
    Función principal de simulación del juego:
      - Movimiento del robot (Rumba)
      - Generación y animación de láseres
      - Aparición de tuercas con animaciones aleatorias
      - Manejo de pausa (ESC) y opción de volver al menú (M)
    """
    rumba_pos = [50, 50]
    velocidad = 5
    score = 0
    ...
    narracion = "Rumba: Robot de Tony Stark, limpiador de tuercas y cazador de láseres."
    ...
    while running:
        ...
        for laser in lasers:
            ...
            if elapsed < laser["warning_duration"]:
                ...
                laser["phase"] = "active"
                # Detección de colisión con Rumba
                if laser["start"][1] == laser["end"][1]:  # Horizontal
                    sy = laser["start"][1]
                    visible_length = 800
        ...
        # Generar tuercas (animación aleatoria)
        for zona in zonas:
            ...
            if count_nuts < max_nuts_per_zone:
                if random.random() < spawn_probability_nut:
                    ...
                    # Escoger aleatoriamente una animación de all_nut_animations
        ...
        # Movimiento Rumba: modo auto o manual
        if not manual_mode:
            min_dist = float("inf")
            for nut in nuts:
        ...
        # Dibujar tuercas con sus animaciones
        for nut in nuts:
        ...
        for laser in lasers:
            ...
            if laser["phase"] == "warning":
                ...
                # Fase activa: se dibuja el láser cortado por obstáculos
                if sy == ey:  # horizontal
                    visible_length = 800
                    for obs in obstacles:
                        if obs["rect"].top <= sy <= obs["rect"].bottom:
                    ...
                    if visible_length < 800:
                        visible_frame = original_surf.subsurface(
                            pygame.Rect(0, 0, visible_length, original_surf.get_height())
                        )
                    ...
                    if visible_length < 650:
                        visible_frame = original_surf.subsurface(
                            pygame.Rect(0, 0, original_surf.get_width(), visible_length)
                        )
        ...
        screen.blit(scoreboard_nut_anim, (scoreboard_icon_x, scoreboard_icon_y))

        # Información de la superficie total y tiempo estimado
...
def main():
    ...
    while True:
        modo = main_menu(screen, clock, ancho_ventana, alto_ventana)
        ...
        # Calcular áreas concurrentemente
        areas = {}
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_zona = {
        ...
        # Cargar frames del Rumba (120x120)
        rumba_filenames = [
        ...
        laser_normal_v_frames = [
            pygame.transform.scale(
                pygame.transform.rotate(surf, 90), (10, 650)
            ) for surf in laser_normal_h_frames
        ]
        # Láser big
        laser_big_h_frames = [
            pygame.transform.scale(surf, (800, 20)) for surf in laser_normal_h_frames
        ]
        laser_big_v_frames = [
            pygame.transform.scale(surf, (20, 650)) for surf in laser_normal_v_frames
        ]
        ...
        # Unir todas las animaciones en una lista
        all_nut_animations = [
            tuerca4_frames,
            tuerca5_frames,
            tuerca6_frames,
        ...
        # Generar obstáculos (concurrencia)
        obstacles = []
        ...
        if ret == "menu":
            # Si se retorna "menu", volvemos a este bucle para mostrar el menú otra vez
            continue
        else:
            break
